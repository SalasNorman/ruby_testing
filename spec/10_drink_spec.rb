...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:her_drink) { described_class.new('gasoline') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(her_drink.type).to eq('gasoline')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(her_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:her_drink) { described_class.new }
      

      # remove the 'x' before running this test
      it 'is full' do
        expect(her_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:her_drink) { described_class.new('petrol',8) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(her_drink).not_to be_full
      end
    end
  end
end
